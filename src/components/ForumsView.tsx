...
export const ForumsView: React.FC<ForumsViewProps> = ({ currentUser, onGoBack }) => {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-6 sm:py-8">
      ...
      {selectedForum ? (
        <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col h-[750px] max-h-[85vh]">
          ...
          <div className="p-4 bg-slate-900 border-b border-slate-800 text-white flex items-center justify-between shrink-0">
            <div className="flex items-center space-x-3">
              <button
                ...
                className="flex items-center space-x-1 px-3 py-1.5 rounded-xl bg-slate-800 hover:bg-indigo-600 text-indigo-300 hover:text-white border border-indigo-500/30 text-xs font-bold transition group"
              ...
              <div>
                <div className="flex items-center space-x-2">
                  <span className="text-[10px] text-indigo-300 font-bold bg-indigo-950/80 px-2 py-0.5 rounded-md border border-indigo-800">
          ...
          {/* Messages Stream */}
          <div className="flex-1 p-4 sm:p-5 overflow-y-auto bg-slate-50/60 dark:bg-slate-950/40 space-y-3">
            {messagesLoading ? (
                  ...
                  >
                    ...
                    >
                      <div className="flex items-center justify-between mb-1 space-x-4">
                        <span className={`font-bold text-[11px] ${isMe ? 'text-indigo-100' : 'text-slate-900 dark:text-white'}`}>
          ...
          {/* Send Message Form */}
          <form onSubmit={handleSendMessage} className="p-3.5 sm:p-4 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 flex items-center space-x-2">
            <input
              type="text"
              placeholder="Écrire un message, partager une idée ou répondre..."
              value={newMessageText}
              onChange={(e) => setNewMessageText(e.target.value)}
              className="flex-1 px-4 py-2.5 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-xs sm:text-sm text-slate-900 dark:text-white focus:outline-hidden focus:border-indigo-500 shadow-2xs"
            />
            <button
              type="submit"
              disabled={sendingMessage || !newMessageText.trim()}
              className="flex items-center space-x-1.5 px-4 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-bold shadow-xs transition disabled:opacity-50"
            >
              <span>Envoyer</span>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 mb-6">
            <div className="relative w-full sm:w-72">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-xs text-slate-800 dark:text-white placeholder:text-slate-400 focus:outline-hidden focus:border-indigo-500 shadow-2xs"
              />
            </div>
            ...
            <select
              ...
              className="w-full sm:w-auto px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-xs font-semibold text-slate-800 dark:text-white focus:outline-hidden focus:border-indigo-500 shadow-2xs"
            >
              ...
              {filteredForums.map((forum) => {
                ...
                return (
                  <div
                    ...
                    className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 p-5 shadow-2xs hover:shadow-md hover:border-indigo-500/50 cursor-pointer transition-all flex flex-col justify-between group"
                  >
                    <div>
                      ...
                      <h4 className="text-base font-bold text-slate-900 dark:text-white group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition">
                        {forum.title}
                      </h4>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-9 pr-3 py-2 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-xs text-slate-900 dark:text-white focus:outline-hidden focus:border-indigo-500"
                    />
                  </div>
                </div>
            ...
            <div className="p-6">
              ...
              <form onSubmit={handleCreateForum} className="space-y-4">
                ...
                <div className="p-3.5 bg-slate-50 dark:bg-slate-800/60 rounded-2xl border border-slate-200 dark:border-slate-700 space-y-3">
                  <span className="block text-xs font-bold text-slate-700 dark:text-slate-300">Type d'accès</span>
                  ...
                  {isPrivate && (
                    <div className="pt-2">
                      ...
                      <input
                        ...
                        className="w-full px-3 py-1.5 bg-white dark:bg-slate-900 border border-amber-300 dark:border-amber-700 rounded-xl text-xs text-slate-900 dark:text-white focus:outline-hidden focus:border-amber-500"
                      />
                    </div>
                  )}
                </div>
